		...
		struct device_attribute *attr, char *buf)
{
	...
	if (IS_ERR_OR_NULL(manager->batt_info)) {
		manager->batt_info = batt_info_find_dev_by_name("batt_info");
		if (IS_ERR_OR_NULL(manager->batt_info)) {
			hq_err("%s:manager->batt_info is_err_or_null\n");
			goto out;
		}
	}

	chip_ok_status = batt_info_get_chip_ok(manager->batt_info);
out:
	return sprintf(buf, "%d\n", chip_ok_status);
}
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	id = batt_info_get_batt_id(manager->batt_info);
out:
	return sprintf(buf, "%d\n", id);
}
...
static ssize_t batt_vendor_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	char *batt_vendor = BATTERY_NAME_UNKNOWN;
	struct power_supply *batt_psy = NULL;
	struct charger_manager *manager = NULL;
	...
	batt_vendor = batt_info_get_batt_name(manager->batt_info);
out:
	return sprintf(buf, "%s\n", batt_vendor);
}

static struct device_attribute batt_vendor_attr =
__ATTR(batt_vendor, 0444, batt_vendor_show, NULL);
